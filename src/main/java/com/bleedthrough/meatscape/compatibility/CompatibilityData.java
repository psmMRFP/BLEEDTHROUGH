...
public final class CompatibilityData {
    ...
    public boolean add(int amount) { int next = Math.max(0, Math.min(MAX, value + amount)); if (next == value) return false; value = next; dirty.run(); return true; }
    public CompoundTag save() { CompoundTag tag = new CompoundTag(); tag.putInt("Compatibility", value); return tag; }
